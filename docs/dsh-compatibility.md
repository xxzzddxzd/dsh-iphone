...
| 通知深链接 | 通知 URL 只知道 session，Web 默认恢复上次选择 | compatibility 7 在 module 启动前把 URL 参数写入 `dsh.sessions.current` |
| 移动端侧栏 | 收起后仍占对话宽度，展开时挤压对话区 | compatibility 7 在标题栏保留单一鲸鱼入口，展开层覆盖对话区 |
...
iOS 上的 Chrome 仍使用 WebKit，因此更换 Chrome 不能绕过 Safari 16 的 JavaScript 能力边界。compatibility 7 入口在任何 DSH module 执行前提供：
...
另外，GFM email autolink 中的 RegExp lookbehind 会让 Safari 16 在解析整个 vendor bundle 时失败。补丁去除该 lookbehind，同时保留调用方已有的 email 边界检查。主 bundle 对 vendor 的 import 使用 `?ioscompat=7`，避免旧 module cache 继续返回解析失败的文件。
