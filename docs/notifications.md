...
三个内容区使用同一组左右两列：左列固定显示 `GOAL`、`进展`、`TOOL`，右列显示内容，进展可占三至四行。进展正文会去掉 Markdown 的标题、引用、代码围栏和链接地址，将列表转换为项目符号，并由原生 `AttributedString` 渲染粗体、删除线和行内代码；解析失败时回退为去除常见标记的纯文本。
