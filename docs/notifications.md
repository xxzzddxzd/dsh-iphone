...
- `进展` 区块按事件时间展示最近一条有效动作或结果：完整的 assistant 信息、工具动作、计划进度和待用户处理内容会依次替换；无目标时最多显示四行，有目标时最多显示三行；
- `TOOL` 区块独立显示最近一次工具调用、执行结果或待授权动作；
- 等待回答、计划确认或操作授权时使用醒目的等待状态和“打开处理”入口。

`assistant/chunk` 的 token 流不会用“模型正在输出”覆盖正文；只有完整且非空的 `assistant/message` 才更新 `进展` 区块。因此模型输出期间，上一条有用信息会一直保留。顶部状态同步显示“正在执行 Bash”“等待操作授权”“正在说明进展”等真实阶段，不显示内部的轮次和步数计数。
