...
同时有多个任务时，只展示最后开始且仍在运行的根任务。该任务结束后，如果较早任务仍在运行，会自动回退展示它。插件同时监听 DSH 权威的 `agent/status`：若一次异常中断没有写入 `turn/end`，agent 回到 idle 时会移除这条未完成任务，避免它继续计时或在较新任务结束后重新占位；已有完整终态的卡片不受影响。全部任务结束后，最后一张卡片冻结执行时长，保留最终 `ASSISTANT` 结果和“查看结果”入口；用户可自行划掉，开始下一项根任务时 broker 会结束旧 Activity 并创建新卡。用户主动取消的 turn 不保留终态卡。subagent 不单独抢占这张卡片。
